...
import { archiveItems, events, flagshipCourse, getCourseProgressSnapshot } from "@/lib/site-data";
        ...
        <div className="content-width grid gap-8 px-4 lg:grid-cols-[minmax(0,1.15fr)_28rem] lg:items-center">
          ...
          <aside className="space-y-4">
            <div className="editorial-card p-6">
              ...
              <h2 className="display-font mt-3 text-3xl text-[var(--soy-brown-900)]">{flagshipCourse.title}</h2>
              <p className="mt-3 text-sm leading-7 text-[var(--soy-ink-soft)]">{flagshipCourse.heroQuote}</p>
              <div className="mt-6 space-y-3">
                {flagshipCourse.heroStats.map((stat) => (
              ...
              <div className="flex items-end justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.14em] text-[var(--soy-ink-muted)]">
                    {progress.currentModuleTitle}
                  </p>
                  <h3 className="display-font mt-2 text-2xl text-[var(--soy-brown-900)]">{progress.currentLessonTitle}</h3>
                </div>
                <span className="tag">{progress.percentComplete}% complete</span>
              </div>
              <div className="mt-4">
            ...
            {events.slice(0, 2).map((event) => (
              <div key={event.id} className="editorial-card p-6">
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-sm text-[var(--soy-ink-soft)]">
                  <span className="tag">{event.datetimeLabel}</span>
